...
python3 scripts/test_v5_1_release.py | tee "$QA/release-static-report.json"
python3 scripts/test_v5_1_ui.py | tee "$QA/ui-static-report.json"
python3 scripts/test_v5_1_accessibility.py | tee "$QA/accessibility-static-report.json"

for file in static/service-worker.js static/assets/js/*.js; do node --check "$file" >/dev/null; done

node scripts/test_v5_1_core.js | tee "$QA/v5.1-core-report.json"
...
core=json.loads((q/'v5.1-core-report.json').read_text())
stress=json.loads((q/'stress-report.json').read_text())
bp=q/'browser-v5.1-report.json'
browser=json.loads(bp.read_text()) if bp.exists() else {'status':'not-run','checks':[],'errors':[]}
summary={
 'status':'ok', 'version':'5.1.0', 'database':'tatadiet-v5','schema_version':1,
 ...
 'v5_1_core':core['status'],
 'accessibility':{'status':a11y['status'],'counts':a11y['counts'],'warnings':len(a11y['warnings'])},
 'stress':stress,
 'browser_qa':browser
}
(q/'v5.1-summary.json').write_text(json.dumps(summary,ensure_ascii=False,indent=2)+'\n')
